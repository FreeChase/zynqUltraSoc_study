...
## 注意事项
1. sdk导入uart默认例程有问题，需要将宏从uart1改成uart0
...
###### 其他CPU请求该数据时
**具体场景：**
cpuA在cache中缓存了地址1的数据，并将cache line 状态改为modify，此时cpuB对地址1发起了访问。

> 具体会发生什么，取决于 CPU B 对地址 1 进行的是**读(Read)** 操作还是 **写(Write)** 操作。以下是底层的运作过程：

**核心机制：总线嗅探（Bus Snooping）**

所有的 CPU 核心都在实时“监听”（嗅探）系统内部总线上的请求。当 CPU B 尝试访问主存中的地址 1 时，它会在总线上广播一个请求。CPU A 监听到这个请求后，检查自己的缓存，发现地址 1 就在自己的 Cache 中，并且状态是“脏”（在 MESI 协议中称为 **Modified / 修改态**），这意味着只有 CPU A 手里有最新的数据，而主存里的数据是过期的。

此时，系统会根据 CPU B 的操作采取以下动作：

**场景一：CPU B 试图“读取”该数据 (Read)**

如果 CPU B 只是想读取地址 1：

1. **拦截读取**：CPU A 监听到总线上的读请求，它会立即向总线发出干预信号，阻止内存控制器把主存里的旧数据发给 CPU B。
2. **数据同步（写回/转发）**：CPU A 必须把这个“脏”数据贡献出来。在传统的实现中，CPU A 会先将数据写回主存，然后主存再发给 CPU B。但在现代微架构中（比如 Intel 的 UPI 或 ARM 的 AMBA），为了降低延迟，通常会触发 **Cache-to-Cache Transfer（缓存间直接传输）**，CPU A 会直接把最新数据发送给 CPU B，同时可能将数据写回主存更新。
3. **状态转变**：
* 此时，主存的数据被更新了（不再是过期数据）。
* CPU A 中该 cache line 的状态从 **Modified（修改，独占且脏）** 降级为 **Shared（共享，干净）**。
* CPU B 接收到数据存入自己的 cache，状态也标记为 **Shared（共享）**。



**场景二：CPU B 试图“写入/修改”该数据 (Write)**

如果 CPU B 想要对地址 1 写入新的数据，它会在总线上发出一个称为“读并获取所有权”（Read-For-Ownership, RFO）的特殊请求：

1. **数据交接**：同样，CPU A 监听到请求，必须先将最新的数据提供给 CPU B（通常通过缓存间传输），以确保 CPU B 是在当前最新数据的基础上进行修改。
2. **强制失效**：因为 CPU B 马上要修改这个数据了，CPU A 手里的副本即将变成废纸。因此，CPU A 会强制将其 cache line 的状态从 **Modified（修改）** 变为 **Invalid（无效）**。
3. **状态转变**：
* CPU B 拿到数据并完成自己的写入操作后，其 cache line 状态直接变为 **Modified（修改）**，成为了这行数据新的唯一“主人”（并且也是“脏”的，没有写回主存）。
